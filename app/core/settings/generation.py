...
class LlmSettings(BaseModel):
    ...
    provider: str = Field(default="mock", min_length=1, description="LLM provider 注册名")
    model: str = Field(default="mock-grounded-json", min_length=1, description="模型标识")
    base_url: str | None = Field(default=None, description="完整 Chat Completions endpoint")
...
class QueryPlanningSettings(BaseModel):
    ...
    strategy: str = Field(default="rule_based", min_length=1, description="QueryPlanner 注册名")
    multi_query_enabled: bool = Field(default=False, description="是否执行多个互补 query")
    max_additional_queries: int = Field(default=2, ge=0, le=8, description="额外 query 上限")
    hyde_enabled: bool = Field(default=False, description="是否允许生成 HyDE 检索辅助文本")
...
class AnswerGenerationSettings(BaseModel):
    ...
    prompt_version: str = Field(default="rag_answer_v2", min_length=1, description="Prompt 版本")
    default_language: str = Field(default="中文", min_length=1, description="默认回答语言")
...
class GenerationSettings(BaseModel):
    ...
    answering: AnswerGenerationSettings = Field(default_factory=AnswerGenerationSettings)
