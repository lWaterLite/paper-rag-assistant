...
| `ContextSegment` | ContextPacker | 最终文本段、完整 source chunk id、实际展示文本可精确映射的原始字符范围、页码范围、章节、token 数 | ContextCitation、PackedContext、后续引用校验 |
| `PackedContext` | ContextPacker | 上下文文本、ContextCitation、已用/丢弃 chunk、segments、token usage | AnswerGenerator |
...
ContextPackRequest(question, EvidenceCandidate[])
  ...
  -> 构建 ContextSegment、ContextCitation、ContextTokenUsage
  -> PackedContext
```
...
当相邻 chunk 合并时，`ContextSegment.source_chunk_ids` 与 `source_ranges` 会保留所有来源及原始字符范围；发生 token 截断时，直接可映射的来源范围会收窄到实际展示的文本前缀，无法精确映射的位置不会被伪造。ContextPacker 先生成检索领域的 `ContextCitation`，AnswerGenerator 再将其转换为最终回答的 `Citation`；两者都为兼容回答格式引用首个 chunk。后续回答级 Citation 校验应以 `ContextSegment` 的完整来源为准，而不是误以为合并段只来自首个 chunk。
